...
### A-Phase 3: Personalized Notifications
- [x] citizenos/src/lib/personalization.ts — Profile-to-content matching engine: maps visa status, employment, household, age to relevant categories/keywords; buildUserRelevance(), scoreRelevance() (0–100), buildRelevanceReason()
- [x] citizenos/src/api/notifications.ts — Profile-driven notification generation: scans bills + actions, scores against user profile, generates unique notifications with deduplication (seenContentIds), respects preferences, supports bill_alert/status_change/action_alert/rep_voted types
- [x] citizenos/src/stores/useNotifStore.ts — generateFromProfile() triggers matching engine on first load, fetches all bills + actions, generates notifications only once per session, polling refreshes unread count
...
- [x] citizenos/src/components/billbreaker/NotificationDropdown.tsx — Notification list with impact-level borders, relevance reason tag, time ago, mark read, navigates to /bill or /action based on type, onboarding prompt pinned at top for incomplete profiles
- [x] citizenos/src/components/billbreaker/NotifPreferences.tsx — Notification type toggles (bill_alerts, action_alerts, status_changes, rep_votes), category toggles, delivery options (high_impact_only, push, email digest)
- [x] Wire NotificationBell into Header.tsx
- [x] Notifications only generated for users who completed onboarding — no notifications shown for incomplete profiles
...
### Matching Engine
- [x] citizenos/src/lib/personalization.ts — Core personalization module:
  - Visa status → category/keyword mapping (21 visa types mapped to relevant policy areas, e.g. H-1B → immigration, labor, economy + keywords like "h-1b", "visa fee", "uscis")
  - Employment status → category/keyword mapping (13 statuses, e.g. self_employed → economy, tax + "small business", "tax deduction")
  - Household situations → category/keyword mapping (11 situations, e.g. parent → education, healthcare + "child tax credit", "childcare")
  - Age group → category/keyword mapping (e.g. 65+ → healthcare, economy + "medicare", "social security")
  - buildUserRelevance() — aggregates all profile data into a unified {categories, keywords, personas} relevance object
  - scoreRelevance() — scores content 0–100 based on category overlap (40pts), keyword hits (30pts), persona match (15pts), state (5pts), impact (10pts)
  - buildRelevanceReason() — generates human-readable "Relevant to you: h-1b, visa fee" strings

### Profile-Driven Notifications
- [x] Notifications generated by scanning all bills + actions against user profile — not static mocks
- [x] Deduplication via seenContentIds — each bill/action generates at most one notification
- [x] Score threshold (>=20) prevents low-relevance spam
- [x] Respects user preferences (high_impact_only, bill_alerts, action_alerts toggles)
- [x] Each notification includes a "reason" field explaining why it's relevant to the user
- [x] Notifications link to /bill/:id or /action/:id based on content type

### Personalized Dashboard
- [x] "Affecting You" section — shows top 5 relevant unread/high-impact notifications with relevance reasons
- [x] "Government Actions For You" — ActionFeed filtered by user personas and categories
- [x] Onboarding prompt card shown when profile is incomplete — explains value of personalization
- [x] Dashboard sections only show personalized labels when onboarding is complete
